...
const ActionMoviesSlider = ({movieDetails}) => {
    ...
    useEffect(() => {
        if ( actionMoviesScrl.current && actionMoviesScrl?.current?.scrollWidth === actionMoviesScrl?.current?.offsetWidth) {
    ...
    const actionMoviesSlide = (shift) => {
        ...
        if (Math.floor(actionMoviesScrl.current.scrollWidth - actionMoviesScrl.current.scrollLeft) <= actionMoviesScrl.current.offsetWidth) {
          setActionMoviesScrollEnd(true);
        } else {
          setActionMoviesScrollEnd(false);
        }
